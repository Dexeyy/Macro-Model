...
## Excel Dashboard
### Factor construction updates

The factor-building pipeline supports three configurable enhancements in `config/config.py`:

- Sign discipline (`SERIES_SIGN`): map each base series to +1 or −1 depending on whether “higher is better” for macro/markets. Any series not listed defaults to +1. The sign is applied before z-scoring so that composites align in direction.

- Correlation pruning (`PRUNE_CORR_THRESHOLD`): after computing robust rolling z-scores, highly redundant series are greedily pruned (keep the strongest by coverage×volatility; drop others with |corr| ≥ threshold).

- Sub-bucket weighting (`SUB_BUCKETS`, `MIN_SUB_BUCKET_COVERAGE`): series are grouped into thematic sub-buckets (e.g., Growth→activity/labour/sentiment). Each sub-bucket contributes its own mean z-score (masked when fewer than the minimum series are available), and the factor averages the sub-bucket means. This prevents overweighting categories that have many series.

If no sign map or sub-buckets are provided, the builder reverts to the previous equal-weighted average of z-scores.
